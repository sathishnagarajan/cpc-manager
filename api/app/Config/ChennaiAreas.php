...
/**
 * Chennai Areas Data
 * Comprehensive list of Chennai localities with pincodes
 */
class ChennaiAreas
{
    ...
    /**
     * Get areas as simple list for dropdown
     */
    public static function getAreasList(): array
    {
        $areas = self::getAreas();
        $list = [];
    ...
    /**
     * Get pincode for an area
     */
    public static function getPincodeForArea(string $area): ?string
    {
        $areas = self::getAreas();
        
        foreach ($areas as $item) {
            if (strcasecmp($item['area'], $area) === 0) {
                return $item['pincode'];
            }
        }
     ...
     */
    public static function searchAreas(string $query): array
    {
        $areas = self::getAreas();
        $results = [];
        ...
        foreach ($areas as $item) {
            if (stripos($item['area'], $query) !== false) {
                $results[] = $item;
            }
        }
